...
export interface WSLBridgeOptions {
  distro?: string;    // 使用するWSLディストリビューション
  cwd?: string;       // 作業ディレクトリ (Windowsパス)
 ...
 * WSL2内でmiyabiコマンドを実行
 */
export function runMiyabiInWSL(args: string[], options: WSLBridgeOptions = {}): number {
  if (!isNativeWindows()) {
 ...
 */
export function execInWSL(command: string, options: WSLBridgeOptions = {}): {
  success: boolean;
  stdout: string;
  stderr: string;
} {
 ...
 * WSL2内にMiyabiをインストール
 */
export function installMiyabiInWSL(distro?: string): boolean {
